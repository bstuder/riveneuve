...
$(function(){$("#form_mail_update").submit(function(a){a.preventDefault();var b=$("#new_mail").val(),c=$("#error_nmail");$.post("/submit_new_mail",{nmail:b},function(b){var a="ok"==b.substring(0,2)?"is_info":"is_error";c.attr("class",a).html(b.substring(2))}).done(function(){$("#curr_mail").val(b)})})});
$(function(){$("#signup").submit(function(a){a.preventDefault();a=$("#abrege").val();var b=$("#complete").val(),c=$("#mail").val(),d=$("#isadmin").prop("checked");d&&!confirm("Voulez-vous vraiment cr\u00e9er un utilisateur avec les droits administrateur?")||$.post("/submit_signup",{name:a,username:b,mail:c,isadmin:d},function(a){var c=$("#error_signup");"ok"==a.substring(0,2)?($("#regusers").append("<div name="+b+"><li>"+b+"</li></div>"),$("#submit_reset").append("<option>"+b+"</option>"),$("#submit_delete").append("<option>"+
b+"</option>"),$(d?"#submit_remove":"#submit_add").append("<option>"+b+"</option>"),c.attr("class","is_info")):c.attr("class","is_error");c.html(a.substring(2));$("#isadmin").prop("checked",!1)})});$("#resetpass").submit(function(a){a.preventDefault();confirm("Voulez-vous vraiment r\u00e9initialiser le mot de passe de cet utilisateur?")&&$.post("/submit_reset_password",{username:$("#submit_reset").val()},function(b){var a="ok"==b.substring(0,2)?"is_info":"is_error";$("#error_reset").attr("class",
